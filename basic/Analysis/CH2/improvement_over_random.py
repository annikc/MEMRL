...
def old_plot_throttled_performance(save=False):
    fig, ax = plt.subplots(2,2,figsize=(15,6), sharex='col', gridspec_kw={'width_ratios': [1, 3]})
    if env[-2:] == '51':
        rwd_colrow= (16,9)
    else:
        rwd_colrow=(14,14)
    ...
    for r, rep in enumerate(reps_to_plot):
        ...
        for j, pct in enumerate(pcts_to_plot):
            v_list = list(gb_che.get_group((env, rep, cache_limits[env][pct])))
            ...
            print(env, rep, pct, v_list)
            avg_, std_ = get_avg_std(v_list,cutoff=5000, smoothing=100)
            avg_rand, std_rand = np.mean(avg_), np.mean(std_)
            ax[0,1].bar(2*r+0.35*j,avg_cos-avg_rand, width=0.35, color=convert_rep_to_color[rep], alpha=pct/100)
    ...
    if save:
        format = 'svg'
        plt.savefig(f'../figures/CH2/improvement_over_random_{version}.{format}', format=format)
    plt.show()


def plot_improvement_over_random_all(gb,gb_rf, envs_to_plot,reps_to_plot,pcts_to_plot,grids,save=False):
    fig, ax = plt.subplots(len(envs_to_plot),2,figsize=(12,3*len(envs_to_plot)), sharex='col', gridspec_kw={'width_ratios': [1, 3]})
    for i, env in enumerate(envs_to_plot):
        ...
        for r, rep in enumerate(reps_to_plot):
            ...
            for j, pct in enumerate(pcts_to_plot):
                ...
                avg_, std_ = get_avg_std(v_list,cutoff=5000, smoothing=100,normalization_factor=norm)
                avg_rand, std_rand = np.mean(avg_), np.mean(std_)
                scale = std_cos/std_rand
                ax[i,1].bar(2*r+0.35*j,(avg_cos-avg_rand)/scale,width=0.35, color=convert_rep_to_color[rep], alpha=pct/100)
    ...
    p50 = mpatches.Patch(color='gray',alpha=.5, label='50')
    p25 = mpatches.Patch(color='gray',alpha=.25, label='25')
    plt.legend(handles=[p100,p75,p50,p25], bbox_to_anchor=(0.5, len(envs_to_plot)*1.15), loc='lower center', ncol=4, title='Episodic Memory Capacity (%)')
    if save:
        format = 'svg'
        plt.savefig(f'../figures/CH2/improvement_over_random_all.{format}', format=format)
    plt.show()
...
plot_improvement_over_random_all(gb_che,gb_rf,envs_to_plot,reps_to_plot,pcts_to_plot,grids,save=False)