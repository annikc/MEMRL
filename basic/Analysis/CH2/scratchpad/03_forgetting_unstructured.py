...
reps_to_plot = ['random', 'onehot'] # df.representation.unique()
...
def plot_throttled_performance(gb,envs_to_plot,reps_to_plot,pcts_to_plot,grids,save=False):
    fig, ax = plt.subplots(len(envs_to_plot),2,figsize=(8,3*len(envs_to_plot)), sharex='col', gridspec_kw={'width_ratios': [1, 2]})
    ...
    ax[0,1].set_title("Limited Capacity with Unstructured Representations")
    if save:
        format = 'svg'
        plt.savefig(f'../figures/CH2/forgetting_unstructured.{format}', format=format)
    plt.show()
