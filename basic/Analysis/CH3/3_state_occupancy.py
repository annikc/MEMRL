...
def occupancy_plot(env_name, rep, pcts_to_plot):
    ...
    for p, pct in enumerate(pcts_to_plot):
        ...
        for item in env.obstacle:
            ...
            EC_occ[coord] = np.nan
            MF_occ[coord] = np.nan
        avg_visit_freq = 1/384
        a= ax[0,p].imshow(EC_occ/avg_visit_freq,cmap='Spectral_r', vmin=0,vmax=np.floor(max/avg_visit_freq))
        ax[0,p].get_xaxis().set_visible(False)
        ax[0,p].get_yaxis().set_visible(False)

        ax[0,p].set_title(f'{pct}')
        b = ax[1,p].imshow(MF_occ/avg_visit_freq,cmap='Spectral_r',vmin=0, vmax=np.floor(max/avg_visit_freq))
