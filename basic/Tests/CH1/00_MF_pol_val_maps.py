...
print(args)
write_to_file = 'shallowAC_withPVmaps.csv'
...
AC_head_agent = nets.shallow_ActorCritic(input_dims, hidden_dims=400, output_dims=env.action_space.n, lr=learning_rate)
...
run = expt(agent, env)
run.run(NUM_TRIALS=num_trials, NUM_EVENTS=num_events, snapshot_logging=True)
...
test_run.run(NUM_TRIALS=num_trials*2,NUM_EVENTS=num_events, snapshot_logging=True)
