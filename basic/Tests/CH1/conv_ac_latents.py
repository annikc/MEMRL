import numpy as np
import matplotlib.pyplot as plt
import gym
import pickle
import torch
import sys
sys.path.append('../../modules')
from modules.Agents.Networks import flat_ActorCritic as head_AC
from modules.Agents.RepresentationLearning.learned_representations import convs, onehot, place_cell, latents
from modules.Agents import Agent

from modules.Experiments import expt
sys.path.append('../../../')
# network from torch.load
version = 1
env_id = f'gridworld:gridworld-v{version}'
example_run_ids = ['c34544ac-45ed-492c-b2eb-4431b403a3a8',
                   '9a12edd8-a978-4e6b-a9f9-e09e0e35c534',
                   '32301262-cd74-4116-b776-57354831c484',
                   'b50926a2-0186-4bb9-81ec-77063cac6861',
                   '15b5e27b-444f-4fc8-bf25-5b7807df4c7f'] # example of each gridworld - v1, v2, v3, v4, v5 using conv representations (no rewards in input tensor)
run_id = example_run_ids[version]
save = False

# make gym environment
env = gym.make(env_id)
plt.close()


reps, name, dim, _ = latents(env, f'./../../Data/agents/{run_id}.pt' )

from modules.Agents.Networks.load_network_weights import load_saved_head_weights
flat_agent = load_saved_head_weights(head_AC(400, 4, 0.0005),f'./../../Data/agents/{run_id}.pt')
print(flat_agent)

# load in agent weights for full conv network
network = torch.load(f'./../../Data/agents/{run_id}.pt')
# states from learned representations
state_reps, name, dim, _ = convs(env)


# compare conv net and head net
index = 10
print(network(state_reps[index]))
print(network.h_act.detach().numpy()[0][0:15])

print(reps[index][0:15])
print(flat_agent(reps[index]))

empty_net = head_AC(network.h_act.shape[1], network.output[0].out_features, network.lr)
onehot_reps, name, dim, _ = onehot(env)
agent = Agent(empty_net, state_representations=reps)
test_env = gym.make(f'gridworld:gridworld-v{version}1')
num_trials = 25000
num_events = 250
write_to_file = 'flat_ac_training.csv'

ex = expt(agent, test_env)
ex.run(num_trials,num_events)
ex.flat_record_log(env_name=env_id, representation_type = 'conv_latents',
                  n_trials=num_trials, n_steps=num_events,
                  dir='../../Data/', file=write_to_file)

for k, v in ex.data.items():
    print(k, len(v))

plt.figure()
plt.plot(ex.data['total_reward'])
plt.show()



# map similarity
sims = np.zeros(env.shape)
reference = latents[90]
for j in range(latents.shape[0]):
    dist = np.linalg.norm(reference-latents[j])
    coord = env.oneD2twoD(j)
    sims[coord] = dist

plt.figure()
plt.imshow(sims, interpolation='none')
plt.show()
### JUNKYARD
'''
new_net = head_AC(network.h_act.shape[1], network.output[0].out_features, network.lr)
new_state_dict = {}
for key in network.state_dict().keys():
    if key[0:6] == 'output':
        if key[7] == '0':
            new_key = 'pol'+key[8:]
            new_state_dict[new_key] = network.state_dict()[key]
        elif key[7] == '1':
            new_key = 'val'+key[8:]
            new_state_dict[new_key] = network.state_dict()[key]

new_net.load_state_dict(new_state_dict)



#test rep
latents = np.zeros((env.nstates, env.nstates))
latents[:] = np.nan # >>????
for index, inp in state_reps.items():
    # do a forward pass
    network(inp)
    # get hidden_layer activity
    latents[index] = network.h_act.detach().numpy()

if save:
    # save latent state reps
    x = latents.copy()
    with open(f'./../../modules/Agents/RepresentationLearning/Learned_Rep_pickles/conv_{env_id[10:]}.p', 'wb') as savedata:
        pickle.dump(x, savedata)



'''
