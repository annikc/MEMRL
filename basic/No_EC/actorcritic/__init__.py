...
class ActorCritic(torch.nn.Module):
	...
	def select_action(self, policy, value):
		a = Categorical(policy) # No episodic 
		action = a.sample() 
	...
	def select_ec_action(self, mf_policy_, mf_value_, ec_policy_):
		...
		b = Categorical(mf_policy_)  # select from episodic store as if from model-free
		action = a.sample() # select action using episodic
		self.saved_actions.append(SavedAction(b.log_prob(action), mf_value_)) # boost the model free - save model free actions
	...
	def finish_trial(self, **kwargs):
		policy_losses = []
		value_losses  = []
		saved_actions = self.saved_actions # stored 
		# MONTE CARLO ASPECT
		returns_ = torch.Tensor(discount_rwds(np.asarray(self.saved_rewards), gamma=self.gamma)) # compute returns
		...
		for (log_prob, value), r in zip(saved_actions, returns_):
			...
			print(value.shape, return_bs.shape) # shape error - you can pass batch 
			value_losses.append(F.smooth_l1_loss(value, return_bs))

		# doesn't matter for MC - successor representation
		if self.use_SR:
	...
	# using off-policy learning - model free is ignored 
	# good seperation and good state representation
	# the policy that is produced by the episodic memory wont be the same as model-free
	#  
