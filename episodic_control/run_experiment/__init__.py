...
class Experiment(object):
    ...
    def policy_arbitration(self,reward_tminus1):
        old_mfcs = self.MF_cs
        if reward_tminus1 <0:
            self.MF_cs = (self.decay*self.MF_cs)
        else:
            self.MF_cs = ((self.decay*self.MF_cs)) + self.alpha*reward_tminus1
        #print(f'polarb: \n decay{self.decay},\n oldmfcs {old_mfcs},\n current mfcs {self.MF_cs},\n alpha {self.alpha},\n last reward {reward_tminus1}')
        if self.MF_cs > 1.0:
            self.MF_cs = 1.0
        elif self.MF_cs < 0.0:
            self.MF_cs = 0.0
    ...
    def run(self, NUM_TRIALS, NUM_EVENTS, **kwargs):
        ...
        for trial in range(NUM_TRIALS):
            ...
            for event in range(NUM_EVENTS):
                ...
                self.data['all_r'].append(reward)### try
